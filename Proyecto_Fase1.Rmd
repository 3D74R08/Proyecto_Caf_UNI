...
variables <- data.frame(
  No = 1:17,  # Columna de enumeración
  Variable = c(
  ...
  Tipo = c(
    "Categórica", "Categórica", "Categórica", "Numérica", "Numérica",
    "Categórica", "Numérica", "Categórica", "Categórica", "Categórica",
    "Numérica", "Numérica", "Categórica", "Categórica", "Numérica", "Numérica",
    "Categórica"
  ),
  Descripción = c(
    "Identificador único del segmento",
    "Nombre de la ciudad (ej. 'Colombo, Sri Lanka')",
    "Distrito o barrio administrativo",
    "Latitud del segmento",
    "Longitud del segmento",
    "ID de la cuenca hidrológica",
    "Elevación en metros, afecta el drenaje",
    "Fuente del modelo digital de elevación (ej. Copernicus_GLO-30_v2023)",
    "Uso del suelo (ej. Residential, Industrial, Roads, Green)",
    "Grupo de suelo basado en infiltración (A: alta, D: baja)",
    "Densidad de drenaje en km/km²",
    "Proximidad al drenaje de tormenta en metros",
    "Tipo de drenaje (ej. CurbInlet, OpenChannel, None)",
    "Fuente de datos de precipitación (ej. ERA5, IMD)",
    "Intensidad histórica de lluvia en mm/h",
    "Período de retorno de eventos de lluvia (ej. 5, 25, 100)",
    "Etiquetas de riesgo (múltiple, ej. monitor, ponding_hotspot, low_lying)"
  )
)

# Generar la tabla con kableExtra
kable(variables, booktabs = TRUE, longtable = TRUE, caption = "Descripción de las variables") %>%
  kable_styling(latex_options = c("striped", "hold_position"), font_size = 10) %>%
  column_spec(1, width = "1cm") %>%  # Ancho para la columna de enumeración
  column_spec(2, width = "3.5cm") %>%  # Ancho para Variable
  column_spec(3, width = "2.5cm") %>%  # Ancho para Tipo
  column_spec(4, width = "8cm")       # Ancho ajustado para Descripción
```
